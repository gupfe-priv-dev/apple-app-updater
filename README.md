...
A cask has exactly one URL — no `mirror` stanza, no fallback list — so when its
host stalls there is nothing to route around it, and the run sits there. A
package host that accepts a TCP connection and then never completes its TLS
handshake will hold up every package queued behind it for as long as you let it.

UpdateAll gives its own `brew` downloads a curl config, via `HOMEBREW_CURLRC`
set **on the child process only** — your own `brew` in a terminal is untouched,
and nothing is written to a shell profile:
...
Deliberately no `max-time`. A total cap can't tell a slow download from a dead
...
file that doesn't exist and brew fails outright with "Failed to parse curl
version" — not just for downloads. UpdateAll checks the file is there before
...
A tip only offers a setting when changing it could actually help. brew passes
curl's stderr through, so the exit code says *why* a download failed: `28` is a
timeout — the transfer was alive and we stopped it, so more time might have
finished it, and the tip links to the threshold. A connection error (`7`, `35`,
`6`, …) means the host refused or dropped us; no amount of extra time would
have helped, so that tip says the mirror is down and offers no setting at all.
The real LibreOffice failure was a `35`.

### Release notes

Generated by [git-cliff](https://git-cliff.org) from the commit subjects in the
tag range, and **only `feat:` and `fix:` subjects make it in**:

| Subject | In the notes |
|---|---|
| `fix: report a failed cask download as failed` | **Fixed** — Report a failed cask download as failed |
| `feat(settings): add a Code Signing row` | **Added** — **settings:** Add a Code Signing row |
| `feat!: rename the bundle identifier` | **Added** — **Breaking:** Rename the bundle identifier |
| `docs: …`, `chore: …`, `refactor: …`, or no prefix | — dropped |

So the notes describe what changed for someone *using* the app, and the
decision is made once, when the commit is written, rather than by editing a
changelog later. A release that genuinely contains no user-facing change says
so instead of listing its own plumbing.

See what the next release would say before cutting it:
